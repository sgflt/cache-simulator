...
public class MainGUI extends javax.swing.JFrame implements Observer {
	 ...
	 *            cesta k jar souboru
	 */
	private void loadCachesFromJar(final String path) {
		final List<String> cacheClassNames = GlobalMethods.getJarClassNames(path,
		...
		for (final String cacheName : cacheClassNames) {
			...
			try {
				...
				final ICache newCache = (ICache) (Class.forName(
						cacheName.replace("/", "."), true, ucl)).newInstance();
				final String[] names = newCache.cacheInfo().split(";");
				final javax.swing.JCheckBox novy = new javax.swing.JCheckBox(
						names[1], false);
	 ...
	 *            cesta
	 */
	private void loadCaches(final String path) {
		final String sep = System.getProperty("file.separator");
		final String pathToPackage = (path + "cz" + sep + "zcu" + sep + "kiv" + sep + "cacheSimulator" + sep
				 + "cachePolicies");
		final File dir = new File(pathToPackage);
		final File[] caches = dir.listFiles();
		for (final File cache : caches) {
			if (cache.isFile() && !cache.getName().contains("ICache")
					&& !cache.getName().contains("$")) {
				try {
					try {
						final ICache newCache = (ICache) (Class
								.forName("cz.zcu.kiv.cacheSimulator.cachePolicies."
										+ cache.getName().substring(
												0,
												cache.getName().lastIndexOf(
														".class"))))
								.newInstance();
						final String[] names = newCache.cacheInfo().split(";");
						final javax.swing.JCheckBox novy = new javax.swing.JCheckBox(
